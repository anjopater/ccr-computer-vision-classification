...
from utils.data_loader import load_image_paths, create_folds
from utils.feature_extractor import extract_cnn_features, apply_pca
from utils.evaluator import train_and_evaluate, cross_validation_with_pca
from utils.logger import save_results
from utils.save_plots import plot_and_save_pca, plot_and_save_explained_variance, plot_and_save_correlation_heatmap, generate_plots_and_save_results, plot_cv_indices
import os
import warnings
...
def main():
    ...
    folds = create_folds(image_paths)

            # Display fold information for verification
    for fold_idx, (fold_images, fold_labels) in enumerate(folds):  # Use enumerate instead of items
        print(f"Fold {fold_idx + 1}:")
        for i in range(0, len(fold_images), 30):  # Group by 30 images per class
            class_label = fold_labels[i]  # Assuming the class label is consistent within the group of 30
            print(f"  Class {class_label}: {fold_images[i:i+30]}")
    ...
    for model_name, model_key in MODELS.items():
        results[model_name] = {}
        train_features = extract_cnn_features(folds, model_key)
        # test_features = extract_cnn_features(folds, test_images, model_key)
        # train_features_array = np.array(train_features)

        print(f"Shape of train_features: {train_features}")
        ...
        for n_components in PCA_COMPONENTS:
            ...
            # train_features_pca, test_features_pca, pca, scaler = apply_pca(train_features, None, n_components)

            # print(f"Shape of train_features_pca: {train_features_pca.shape}")
            cross_validation_with_pca(train_features,n_components )
